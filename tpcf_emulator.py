"""
GP emulation for two-point correlation function xi(r).
Similar to ps_emulator.py.
"""
...
# Load data
params = np.load('./data/tpcf_cosmo.npy')
xi_data = np.load('./data/tpcf.npy')

X_train, X_test, y_train, y_test = train_test_split(params, xi_data, test_size=0.2)
...
# r array
r = np.logspace(np.log10(15), np.log10(120), y_true.shape[1])
...
for i in range(n_show):
    ax1.plot(r, y_true[i], color=colors[i], label=f'True {i+1}')
    ax1.plot(r, y_pred[i], '--', color=colors[i], label=f'Pred {i+1}')
ax1.set_xlabel('r (Mpc/h)')
ax1.set_ylabel('xi(r)')
ax1.set_xscale('log')
ax1.legend()
ax1.set_title('Correlation functions (true vs predicted)')

for i in range(n_show):
    rel_err = np.abs(y_pred[i] - y_true[i]) / (np.abs(y_true[i]) + 1e-8)
    ax2.plot(r, rel_err, color=colors[i], label=f'Error {i+1}')
...
plt.tight_layout()
plt.savefig('./fig/tpcf_emulation.png', dpi=150)
print("Figure saved to ./fig/tpcf_emulation.png")
plt.show()
